...
import { CheckCircle2, Eye, FileText, FolderOpen, RotateCcw, Search, XCircle } from "lucide-react";
...
export function ApplicationApprovalsView() {
    ...
    return (
        <AdminLayout title={UI_TEXT.applicationApprovals.title} subtitle={UI_TEXT.applicationApprovals.subtitle}>
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <div className="relative w-64 sm:w-72">
                                    ...
                                    <input
                                        type="text"
                                        placeholder={UI_TEXT.applicationApprovals.filterSearchPlaceholder}
                                        value={searchQuery}
                                        onChange={(e) => setSearchQuery(e.target.value)}
                                        className="h-9 w-full rounded-full border border-slate-200 bg-white pr-4 pl-9 text-xs text-slate-900 placeholder-slate-400 transition outline-none focus:border-wine focus:ring-1 focus:ring-wine"
                                    />
                                </div>
                                ...
                                <select
                                    ...
                                    className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 outline-none focus:border-wine"
                                >
                                    <option value="">{UI_TEXT.applicationApprovals.allCoursesOption}</option>
                                </select>

                                <button
                                    onClick={handleResetFilters}
                                    className="inline-flex h-9 cursor-pointer items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 transition hover:bg-slate-50"
                                >
                            ...
                            <table className="w-full border-collapse text-left text-xs text-slate-700">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/70 text-[11px] font-bold tracking-wider text-slate-500 uppercase">
                                        <th className="px-5 py-3.5 font-bold">{UI_TEXT.applicationApprovals.thCode}</th>
                                        <th className="px-5 py-3.5 font-bold">{UI_TEXT.applicationApprovals.thStudent}</th>
                                        <th className="px-5 py-3.5 font-bold">{UI_TEXT.applicationApprovals.thType}</th>
                                        <th className="px-5 py-3.5 font-bold">{UI_TEXT.applicationApprovals.thCourseSemester}</th>
                                        <th className="px-5 py-3.5 font-bold">{UI_TEXT.applicationApprovals.thSubmittedAt}</th>
                                        <th className="px-5 py-3.5 text-center font-bold">{UI_TEXT.applicationApprovals.thStatus}</th>
                                        <th className="w-[140px] px-5 py-3.5 text-center font-bold">{UI_TEXT.applicationApprovals.thAction}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {paginatedList.map((item) => (
                                        <tr key={item.id} className="group border-b border-slate-100 transition duration-150 hover:bg-slate-50/80">
                                            <td className="px-5 py-3.5 font-extrabold text-slate-900">
                                                <span className="rounded-lg bg-slate-100 px-2 py-1 font-mono text-[11.5px] text-slate-800">{item.code}</span>
                                            </td>
                                            <td className="px-5 py-3.5">
                                                <div className="text-[13.5px] font-bold text-slate-900">{item.student.fullName}</div>
                                            ...
                                            <td className="px-5 py-3.5">
                                                <span className="inline-flex rounded-lg border border-indigo-100 bg-indigo-50 px-2.5 py-1 text-xs font-bold text-indigo-700">
                                                    {item.typeName}
                                                </span>
                                                {item.examType && (
                                            ...
                                            <td className="px-5 py-3.5">
                                                ...
                                                {item.attachmentName && (
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        className="mt-0.5 inline-flex items-center gap-1 text-[11px] font-bold text-wine hover:underline"
                                                    >
