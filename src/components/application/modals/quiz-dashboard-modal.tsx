...
import { type QuizDashboardModalProps, StudentQuizStatusEnum } from "@/types/session-quiz.types";

export function QuizDashboardModal({ isOpen, onClose, results, activeQuiz, isClosed }: QuizDashboardModalProps) {
    ...
    const totalQuestions = activeQuiz?.questions?.length || results[0]?.totalQuestionsCount || 10;
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                ...
                                <div className="rounded-2xl border border-emerald-100 bg-emerald-50/30 p-4">
                                    ...
                                    <div className="mt-1 text-[11px] font-medium text-emerald-700">
                                        ...
                                        <strong className="font-bold">
                                            {totalQuestions > 0 ? ((Number(avgCorrect) / totalQuestions) * 10).toFixed(1) : 0}
                                            {"/10"}
                                        </strong>
                                    </div>
                                </div>
                                ...
                                <div className="rounded-2xl border border-purple-100 bg-purple-50/30 p-4">
                                    ...
                                    <div className="mt-2 flex items-baseline gap-2">
                                        <span className="text-2xl font-black text-purple-900">{activeQuiz?.durationMinutes || 15}</span>
                                        ...
                                        <table className="w-full text-left text-xs text-slate-600">
                                            ...
                                            <tbody className="divide-y divide-slate-100 font-medium">
                                                {results.map((item, idx) => {
                                                    ...
                                                    const percentage = qTotal > 0 ? Math.round((correct / qTotal) * 100) : 0;
