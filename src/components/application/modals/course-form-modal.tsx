...
export function CourseFormModal({ isOpen, onOpenChange, initialData, onSubmit }: CourseFormModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!title.trim() || !code.trim() || !selectedCategory) {
            toast.error("Lỗi", "Vui lòng nhập đầy đủ Mã môn học, Tên môn học và Công thức tính điểm cũ");
            return;
        }

        if (useCustomFormula) {
            if (generalTotal !== 100) {
                toast.error("Lỗi", `Tổng trọng số môn học phải bằng 100% (Hiện tại: ${generalTotal}%)`);
                return;
            }
            if (hackathonTotal !== 100) {
                toast.error("Lỗi", `Tổng trọng số Hackathon phải bằng 100% (Hiện tại: ${hackathonTotal}%)`);
                return;
            }
            if (finalExamType === FinalExamTypeEnum.PROJECT && projectTotal !== 100) {
                toast.error("Lỗi", `Tổng trọng số thi Project phải bằng 100% (Hiện tại: ${projectTotal}%)`);
                return;
            }
            if (finalExamType === FinalExamTypeEnum.ESSAY && essayTotal !== 100) {
                toast.error("Lỗi", `Tổng trọng số thi Tự luận phải bằng 100% (Hiện tại: ${essayTotal}%)`);
                return;
            }
        }
                                    ...
                                    <>
                                        ...
                                        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition duration-200 hover:shadow-md">
                                            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                                ...
                                                <span
                                                    className={`rounded-full px-3 py-1 text-xs font-bold transition-colors ${generalTotal === 100
                                        ...
                                        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition duration-200 hover:shadow-md">
                                            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                                ...
                                                <span
                                                    className={`rounded-full px-3 py-1 text-xs font-bold transition-colors ${hackathonTotal === 100
                                                            ? "border border-emerald-200 bg-emerald-50 text-emerald-700"
                                                            : "border border-rose-200 bg-rose-50 text-rose-700"
                                                        }`}
                                                >
