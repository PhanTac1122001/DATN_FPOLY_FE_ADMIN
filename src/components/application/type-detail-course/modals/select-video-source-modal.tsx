...
import { ArrowLeft, ChevronRight, FileText, Play, X } from "lucide-react";
import { Button } from "@/components/base/buttons/button";
import { CustomModal, Dialog } from "@/components/ui/custom-modal";
import { UI_TEXT } from "@/constants/ui-text.constants";
import type { LinkVideoModalProps, SelectVideoSourceModalProps } from "@/types/material.types";
import { isValidUrl } from "@/utils/url.utils";

export function SelectVideoSourceModal({ isOpen, onOpenChange, onSelectLink, onSelectFile }: SelectVideoSourceModalProps) {
    return (
            ...
            <CustomModal.Content className="w-full max-w-2xl !rounded-[28px]">
                <Dialog className="relative flex flex-col gap-6 rounded-[28px] bg-white p-7 shadow-2xl outline-none sm:p-9">
                    ...
                    <div className="flex flex-col items-center gap-1 border-b border-slate-100 pb-5 text-center">
                        <h3 className="text-xl font-extrabold text-slate-800">{UI_TEXT.learningMaterials.modalTitleSelectVideo}</h3>
                        <p className="mt-0.5 text-xs font-medium text-slate-400">{UI_TEXT.learningMaterials.modalDescSelectVideo}</p>
                    </div>
                    ...
                    <div className="flex flex-col gap-3.5">
                            ...
                            <div className="flex min-w-0 items-center gap-4">
                                ...
                                <div className="flex min-w-0 flex-col gap-0.5">
                                    <span className="text-sm font-extrabold text-slate-800 transition group-hover:text-wine sm:text-base">
                                        {UI_TEXT.learningMaterials.btnInputVideoLink}
                                    </span>
                                    <span className="text-xs font-medium text-slate-400">{UI_TEXT.learningMaterials.descInputVideoLink}</span>
                                </div>
                            </div>
                        ...
                        >
                            <div className="flex min-w-0 items-center gap-4">
                                ...
                                <div className="flex min-w-0 flex-col gap-0.5">
                                    <span className="text-sm font-extrabold text-slate-800 transition group-hover:text-wine sm:text-base">
                                        {UI_TEXT.learningMaterials.btnUploadVideoFile}
                                    </span>
                                    <span className="text-xs font-medium text-slate-400">{UI_TEXT.learningMaterials.descUploadVideoFile}</span>
                                </div>
                            </div>
                            <ChevronRight className="ml-2 size-5 shrink-0 text-slate-300 transition duration-200 group-hover:translate-x-1 group-hover:text-wine" />
                        </button>
...
export function LinkVideoModal({ isOpen, onOpenChange, tempLink, setTempLink, onBack, onConfirm }: LinkVideoModalProps) {
                ...
                <Dialog className="relative flex flex-col gap-6 rounded-[28px] bg-white p-7 shadow-2xl outline-none sm:p-9">
                    <div className="relative flex items-center justify-between border-b border-slate-100 pb-5">
                        ...
                        <div className="-ml-8 flex flex-1 flex-col items-center pr-4 text-center">
                            <h3 className="text-xl font-extrabold text-slate-800">{UI_TEXT.learningMaterials.modalTitleLinkVideo}</h3>
                            <p className="mt-0.5 text-xs font-medium text-slate-400">{UI_TEXT.learningMaterials.enterVideoOnlinePath}</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-medium text-slate-700">
                                {UI_TEXT.learningMaterials.labelLinkVideoUrl} <span className="text-red-500">{"*"}</span>
                            </label>
                            <input
                                type="text"
                                value={tempLink}
                                onChange={(e) => setTempLink(e.target.value)}
                                placeholder={UI_TEXT.learningMaterials.placeholderLinkVideoUrl}
