...
import { FIRST_DAY_OF_MONTH, LAST_DAY_OF_MONTH_OFFSET, SATURDAY_INDEX, SUNDAY_INDEX } from "@/constants/application.constants";
import type { CalendarCellProps } from "@/types/application.types";
...
export const CalendarCell = ({ date, isHighlighted, ...props }: CalendarCellProps) => {
    ...
    const lastDayOfMonth = new Date(date.year, date.month, LAST_DAY_OF_MONTH_OFFSET).getDate();
    const isLastDayOfMonth = date.day === lastDayOfMonth;
    const isFirstDayOfMonth = date.day === FIRST_DAY_OF_MONTH;
    ...
    return (
        <AriaCalendarCell
            ...
            className={({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, isOutsideMonth }) => {
                const isRoundedLeft = isSelectionStart || dayOfWeek === SUNDAY_INDEX;
                const isRoundedRight = isSelectionEnd || dayOfWeek === SATURDAY_INDEX;
