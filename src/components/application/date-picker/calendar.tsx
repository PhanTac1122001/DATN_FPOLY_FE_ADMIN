...
import { COMPARE_EQUAL, DAY_ABBREV_LENGTH, SLICE_START } from "@/constants/application.constants";
import { UI_TEXT } from "@/constants/ui-text.constants";
import type { CalendarProps } from "@/types/application.types";
...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    return (
        <AriaCalendarContext.Provider
            value={{
                value,
                onChange: (nextValue) => onChange(Array.isArray(nextValue) ? null : (nextValue as DateValue | null)),
                focusedValue,
                onFocusChange: (nextValue) => onFocusChange(Array.isArray(nextValue) ? undefined : nextValue),
            }}
        >
            {children}
        </AriaCalendarContext.Provider>
        ...
        <ContextWrapper>
            <AriaCalendar {...props} className={(state) => cx("flex flex-col gap-3", typeof className === "function" ? className(state) : className)}>
                <header className="flex items-center justify-between">
                    <Button
                        slot="previous"
                        iconLeading={(props) => <ChevronLeft color="currentColor" {...props} size={20} variant="Linear" />}
                        size="sm"
                        color="tertiary"
                        className="size-8"
                    />
                    ...
                    <Button
                        slot="next"
                        iconLeading={(props) => <ChevronRight color="currentColor" {...props} size={20} variant="Linear" />}
                        size="sm"
                        color="tertiary"
                        className="size-8"
                    />
                ...
                <div className="flex gap-3">
                    ...
                    <PresetButton value={today(getLocalTimeZone())}>{UI_TEXT.common.today}</PresetButton>
                </div>
                ...
                <AriaCalendarGrid weekdayStyle="short" className="w-max">
                        ...
                        {(day) => (
                            <AriaCalendarHeaderCell className="p-0">
                                <div className="flex size-10 items-center justify-center text-sm font-medium text-secondary">
                                    {day.slice(SLICE_START, DAY_ABBREV_LENGTH)}
                                </div>
                    ...
                    <AriaCalendarGridBody className="[&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent [&_tr:last-of-type]:border-none">
                        {(date) => (
                            <CalendarCell
                                date={date}
                                isHighlighted={highlightedDates?.some((highlightedDate) => date.compare(highlightedDate) === COMPARE_EQUAL)}
                            />
                        )}
