...
import { DEFAULT_PAGE_INDEX, DEFAULT_TOTAL_PAGES } from "@/constants/application.constants";
import { UI_TEXT } from "@/constants/ui-text.constants";
import { useBreakpoint } from "@/hooks/use-breakpoint";
import type { MobilePaginationProps, PaginationButtonGroupProps, PaginationCardMinimalProps, PaginationProps } from "@/types/application.types";
...
const MobilePagination = ({ page = DEFAULT_PAGE_INDEX, total = DEFAULT_TOTAL_PAGES, className, onPageChange }: MobilePaginationProps) => {
    return (
        <nav aria-label="Pagination" className={cx("flex items-center justify-between md:hidden", className)}>
            ...
            <span className="text-sm text-fg-secondary">
                {UI_TEXT.common.pagination.page} <span className="font-medium">{page}</span> {UI_TEXT.common.pagination.of}{" "}
...
export const PaginationPageDefault = ({ rounded, page = DEFAULT_PAGE_INDEX, total = DEFAULT_TOTAL_PAGES, className, ...props }: PaginationProps) => {
            ...
            <div className="hidden flex-1 justify-start md:flex">
                <Pagination.PrevTrigger asChild>
                    <Button iconLeading={ArrowLeft} color="link-gray" size="sm">
                        {isDesktop ? UI_TEXT.common.pagination.previous : undefined}{" "}
                    </Button>
                ...
                {({ pages, currentPage, total }) => (
                    <>
                        ...
                        <div className="flex justify-center text-sm whitespace-pre text-fg-secondary md:hidden">
                            {UI_TEXT.common.pagination.page} <span className="font-medium">{currentPage}</span> {UI_TEXT.common.pagination.of}{" "}
                            <span className="font-medium">{total}</span>
                        </div>
                    </>
                )}
            ...
            <div className="hidden flex-1 justify-end md:flex">
                <Pagination.NextTrigger asChild>
                    <Button iconTrailing={ArrowRight} color="link-gray" size="sm">
                        {isDesktop ? UI_TEXT.common.pagination.next : undefined}
                    </Button>
...
export const PaginationPageMinimalCenter = ({ rounded, page = DEFAULT_PAGE_INDEX, total = DEFAULT_TOTAL_PAGES, className, ...props }: PaginationProps) => {
    const isDesktop = useBreakpoint("md");
...
export const PaginationCardDefault = ({ rounded, page = DEFAULT_PAGE_INDEX, total = DEFAULT_TOTAL_PAGES, ...props }: PaginationProps) => {
    const isDesktop = useBreakpoint("md");
            ...
            <nav aria-label="Pagination" className={cx("hidden items-center gap-3 md:flex", align === "center" && "justify-between")}>
                <div className={cx(align === "center" && "flex flex-1 justify-start")}>
                    <Button isDisabled={page === 1} color="secondary" size="sm" onClick={() => onPageChange?.(Math.max(0, page - 1))}>
                        {UI_TEXT.common.pagination.previous}
                    </Button>
                </div>
                ...
                >
                    {UI_TEXT.common.pagination.page} {page} {UI_TEXT.common.pagination.of} {total}
                </span>
...
export const PaginationButtonGroup = ({ align = "left", page = DEFAULT_PAGE_INDEX, total = DEFAULT_TOTAL_PAGES, ...props }: PaginationButtonGroupProps) => {
    const isDesktop = useBreakpoint("md");
            ...
            <Pagination.Root {...props} page={page} total={total}>
                <Pagination.Context>
                    {({ pages }) => (
                        <ButtonGroup size="md">
                            <Pagination.PrevTrigger asChild>
                                <ButtonGroupItem iconLeading={ArrowLeft}>{isDesktop ? UI_TEXT.common.pagination.previous : undefined}</ButtonGroupItem>
                            </Pagination.PrevTrigger>
                            ...
                            <Pagination.NextTrigger asChild>
                                <ButtonGroupItem iconTrailing={ArrowRight}>{isDesktop ? UI_TEXT.common.pagination.next : undefined}</ButtonGroupItem>
                            </Pagination.NextTrigger>
                        </ButtonGroup>
                    )}
                </Pagination.Context>
