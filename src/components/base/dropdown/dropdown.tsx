...
import type { ButtonProps as AriaButtonProps, SeparatorProps as AriaSeparatorProps } from "react-aria-components";
import {
...
import { ICON_SIZES } from "@/constants/app.constants";
// Import type for internal use
import type { DropdownItemProps, DropdownMenuProps, DropdownPopoverProps } from "@/types/base-components.types";
import { cx } from "@/utils/cx";

// Re-export type from centralized location
export type { DropdownItemProps, DropdownMenuProps, DropdownPopoverProps } from "@/types/base-components.types";

const DropdownItem = ({ label, children, addon, icon: Icon, unstyled, iconSize, iconVariant, iconColor, ...props }: DropdownItemProps) => {
    if (unstyled) {
    ...
    return (
        <AriaMenuItem
            ...
            className={(state) =>
                cx(
                    "group block cursor-pointer py-px outline-hidden",
        ...
        >
            {(state) => (
                <div
                    className={cx(
                        "relative flex items-center rounded-md px-3 py-2.5 outline-focus-ring transition duration-100 ease-linear",
                ...
                >
                    {Icon && (
                        <Icon
                            ...
                            color={iconColor}
                            className={cx("mr-3 shrink-0", !iconColor && (state.isDisabled ? "text-fg-disabled" : "text-fg-quaternary"))}
                        />
                    )}
...
const DropdownSeparator = (props: AriaSeparatorProps) => {
    return <AriaSeparator {...props} className={cx("my-1 h-px w-full bg-slate-200", props.className)} />;
};
