...
import type { CheckboxBaseProps } from "@/types/base-components.types";
// Import type for internal use
import type { CheckboxProps } from "@/types/base-components.types";
import { cx } from "@/utils/cx";

// Re-export type from centralized location
export type { CheckboxBaseProps } from "@/types/base-components.types";
    ...
    return (
        <div
            className={cx(
                ...
                (isSelected || isIndeterminate) && !isDisabled && "bg-brand-solid ring-bg-brand-solid",
                (isSelected || isIndeterminate) && isDisabled && "bg-brand-solid/50 ring-brand-solid/50",
                isDisabled && !isSelected && !isIndeterminate && "cursor-not-allowed bg-disabled_subtle ring-disabled",
...
// Re-export type from centralized location
export type { CheckboxProps } from "@/types/base-components.types";

export const Checkbox = ({ label, hint, size = "sm", className, boxClassName, validationBehavior = "aria", ...ariaCheckboxProps }: CheckboxProps) => {
    const sizes = {
        sm: {
            root: "gap-2",
            textWrapper: "",
            ...
            {({ isSelected, isIndeterminate, isDisabled, isFocusVisible }) => (
                <>
                    <CheckboxBase
                        ...
                        className={cx(label || hint ? "mt-0.5" : "", boxClassName)}
                    />
                    {(label || hint) && (
